...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('producation.view')) {
                $menu->dropdown(
                    __('lang_v1.producation'), // Dropdown menu label
                    function ($sub) {
                        if (auth()->user()->can('producation.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\RecipeController::class, 'index']), // URL to RecipeController@index
                                __('lang_v1.list_recipes'), // Label for the first menu item
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'recipe' && request()->segment(2) == '']
                            );
                        }
                        if (auth()->user()->can('producation.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\RecipeController::class, 'create']), // URL to RecipeController@create
                                __('lang_v1.producation_add'), // Label for adding a new recipe
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'recipe' && request()->segment(2) == 'create']
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-utensils'] // Icon for the dropdown menu
                )->order(50); // Order of the dropdown menu relative to others
            }
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        if (! empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own'))) {
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseOrderController::class, 'create'], ['status_purchase' => 'quotation']),
                                __('lang_v1.add_quotation'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->get('status_purchase') == 'quotation']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                        if (! empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own'))) {
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseOrderController::class, 'quotation_list']),
                                __('Quotation List'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'quotation-list']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                        ...
                        if (! empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own'))) {
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseOrderController::class, 'create'], ['status_purchase' => 'gate_inward']),
                                __('Add Gate In-Word Pass'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->get('status_purchase') == 'gate_inward']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                        if (! empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own'))) {
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseOrderController::class, 'gate_inward_pass']),
                            __('Gate In-Word Pass'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'gate-inward-pass']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                        if (! empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own'))) {
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseOrderController::class, 'create'], ['status_purchase' => 'add_grn']),
                                __('Add GRN'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->get('status_purchase') == 'add_grn']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                        if (! empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own'))) {
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseOrderController::class, 'grn_list']),
                            __('GRN'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'grn_list']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                    },
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        if (! empty($pos_settings['enable_sales_order']) && ($is_admin || auth()->user()->hasAnyPermission(['so.view_own', 'so.view_all', 'so.create']))) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'create'], ['status' => 'gate_outward']),
                                __('Add Gate Out-Word Pass'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->get('status') == 'gate_outnward']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                        if (! empty($pos_settings['enable_sales_order']) && ($is_admin || auth()->user()->hasAnyPermission(['so.view_own', 'so.view_all', 'so.create']))) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'gate_outward_pass']),
                            __('Gate Out-Word Pass'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'gate-outward-pass']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                        if (! empty($pos_settings['enable_sales_order']) && ($is_admin || auth()->user()->hasAnyPermission(['so.view_own', 'so.view_all', 'so.create']))) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'create'], ['status' => 'add_gdn']),
                                __('Add GDN'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->get('status') == 'add_gdn']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                        if (! empty($pos_settings['enable_sales_order']) && ($is_admin || auth()->user()->hasAnyPermission(['so.view_own', 'so.view_all', 'so.create']))) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'gdn_list']),
                            __('GDN'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'gdn_list']
                                // ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        }
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
