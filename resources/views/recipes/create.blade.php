@extends('layouts.app')
@section('title', __('lang_v1.producation_add'))
@section('css')
    <style>
        .total_cost {
            border: none;
...
@section('content')
    ...
    <section class="content-header">
        <h1>@lang('lang_v1.producation_add')</h1>
    </section>

    <section class="content">
        {!! Form::open([
            'url' => action([\App\Http\Controllers\RecipeController::class, 'store']),
            'method' => 'post',
            'id' => 'recipe_add_form',
            'class' => 'recipe_form',
        ]) !!}
            ...
            <div class="row" id="recipe_fields_container">
                <div class="col-sm-3">
                    <div class="form-group">
                        ...
                        {!! Form::text('recipe_name', null, ['class' => 'form-control', 'placeholder' => __('recipes.recipe_name')]) !!}
                    </div>
                </div>
                <div class="col-sm-3">
                    <div class="form-group">
                ...
                <div class="recipe_row col-sm-12">
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            {!! Form::select('item_code[]', $product_codes, null, [
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            {!! Form::select('name[]', $product_names, null, [
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            <input type="text" name="qty[]" class="form-control qty-input" required
                                placeholder="@lang('recipes.qty')">
                        </div>
                    </div>
                    <div class="col-sm-2">
                        <div class="form-group">
                            <label for="rate">@lang('recipes.rate') :*</label>
                            <input type="text" name="rate[]" class="form-control rate-input" required
                                placeholder="@lang('recipes.rate')">
                        </div>
                    </div>
                    <div class="col-sm-2">
                        <div class="form-group">
                            <label for="cost">@lang('recipes.cost') :*</label>
                            <input type="text" name="cost[]" class="form-control cost-input" placeholder="@lang('recipes.cost')"
                                readonly>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-2" style="display:none">
                        <div class="form-group">
                            ...
                            {!! Form::select('product_id[]', $product_ids, null, [
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="text-center">
                        ...
                        {!! Form::text('total_cost', null, ['id' => 'total_cost_input', 'class' => 'total_cost', 'readonly']) !!}
...
@section('javascript')
    ...
    <script>
        function calculateCost() {
            var totalCost = 0;
            let productQuantity = parseFloat($('input[name="product_quantity"]').val()) || 1;
            console.log('test', productQuantity);
            $('.recipe_row').each(function() {
                var qty = parseFloat($(this).find('.qty-input').val()) || 0;
                var rate = parseFloat($(this).find('.rate-input').val()) || 0;
                var productQty = $(this).find('.product-qty').val() || 0;
                var cost = qty * rate;
                totalCost += cost;
                $(this).find('.cost-input').val(Math.floor(cost));
            });


            totalCost *= productQuantity;
            $('#total_cost').text(Math.floor(totalCost)); // Use Math.floor to get only integer part for total cost display
            $('#total_cost_input').val(Math.floor(totalCost)); // Set the value of the hidden input field
        }
        ...
        $(document).ready(function() {
                ...
                var newRow = `
                    <div class="recipe_row col-sm-12">
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                <label for="qty">@lang('recipes.qty') :*</label>
                                <input type="text" name="qty[]" class="form-control qty-input" required placeholder="@lang('recipes.qty')">
                            </div>
                        </div>
                        <div class="col-sm-2">
                            <div class="form-group">
            ...
            $(document).on('input', '.rate-input', calculateCost);
            $(document).on('input', '.qty-input', calculateCost);
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            if (field === 'item_code') {
                              ...
                              //  $('#total_cost_input').val(Math.floor(totalCost));
                                $('.location').val(product.product_location);
                                $row.find('input[name="rate[]"]').val(product.sell_price_inc_tax);
                                $row.find('input[name="cost[]"]').val(product.cost);
                                $row.find('select[name="product_id[]"]').val(product.id).trigger(
