...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	@if(!empty($is_direct_sell))
  		<br>
  		<textarea style="display:none !important;" class="form-control" name="products[{{$row_count}}][sell_line_note]" rows="2" hidden>{{$sell_line_note}}</textarea>
  		<p class="help-block" hidden><small hidden>@lang('lang_v1.sell_line_description_help')</small></p>
	@endif
	</td>
	...
	@if(!empty($is_direct_sell))
		...
		<td class="text-center {{$hide_tax}}" hidden>
